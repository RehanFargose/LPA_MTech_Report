...
\noindent The studies summarized in Table~\ref{tab:litreview} collectively indicate the flaws in Legal NLP primarily local language support, jurisdiction transferrability, LLM token limits, lack of precedents, etc. The Literature also offers insights into approaches such as DAPT, Contrastive Learning, RAG, UDA, Sliding Sequence window, Vector Databases and other techniques which can help us overcome the stated gaps in Chapter.~\ref{ch:system_analysis}.
...
\textbf{Gap 5: Jurisdictional and Geographic Bias:.} A significant majority of reviewed studies~\cite{b6, b8, b11, b16}; a critical gap exists for models trained on Indian Legal corpora, as the majority of current research focuses on US, EU, or Chinese datasets.
...
\textbf{Gap 10: Diminishing Returns in Training.} There is a significant efficiency gap where marginal gains in F1 score (e.g., 1.5\%) require exponentially longer training times \cite{b9} \cite{b21} (e.g., 7x training time for 1.5\% f1 score gain).
