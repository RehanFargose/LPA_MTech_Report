...
\section{Overview}
The \textbf{Legal Precedent Assistant for Indian Family Courts} project aims to provide a highly accurate modular and transferrable NLP Pipeline/System for ILC. 
The proposed system will be built for Family courts and use Gemini, Llama, Deepseek as baselinses with DAPT for the smaller models on ILC. 
...
\begin{itemize}
    \item The relevant legal documents from both sides should be given as input to the system alongside any pre-existing court judgements.
    \item The files can be uploaded as .docx/.pdf/.txt formats.
\end{itemize}

\textbf{FR2: Translation, Text Cleaning and Preprocessing}
\begin{itemize}
    \item If the document is not in English it will be translated into English using a local model or Google Translate API. 
    \item The system pipeline will then use regex to remove any unnecessary symbols and patterns to convert the input documents into text files for further processing.
\end{itemize}

\textbf{FR3: Evidence Scrutinization}
\begin{itemize}
    \item Gemini should be used to perform evidence scrutinization and a consistency audit for both sides, to try and determine contradictions in both side’s statements.
\end{itemize}

\textbf{FR4: Legal code Mapping}
\begin{itemize}
    \item If the document contains any articles/code/IPC/BNS/section violations they should be tracked either based on the codes or based on statements or by using a vector db to match and extract relevant legal codes.
\end{itemize}

\textbf{FR5: Precedent Retrieval}
\begin{itemize}
    \item Use a vector DB to match the current court case documents with past similar court cases and find precedents from past judgements to aid in verdict/judgement of present case.
    \item To preserve token limit and balance performance use only the top 10 most relevant precedents.
\end{itemize}

\textbf{FR6: Verdict Prediction}
\begin{itemize}
	\item Predict the verdict using the previous 3 requirements while also taking into account retrieved precedents.
...
\begin{itemize}
    \item Make use of freely available data(AWS SC/HC Registry) for building a judgements dataset, vector DBs(Chroma DB), LLM models and LLM APIs.
\end{itemize}
...
\begin{itemize}
    \item The system should be able to handle any cases in the Family courts domain reliably, and should be maintained and upgraded as per changes in Family law such as transition from IPC to BNS. 
\end{itemize}
...
\begin{itemize}
    ...
    \item This is sufficient to train smaller BERT/LLM models alognside PyMuPDF and NLLB(translation model) with small batch size but takes longer times and has low efficiency.
    \item Solution: Consider following more advanced GPUs:
    	\begin{itemize}
    		\item RTX series GPUs.
    		\item Ampere Architecture based GPUs.
    		\item Free T4 GPU access from Google Collab and Kaggle(Preferred).
    	\end{itemize}
...
\begin{itemize}
    \item The current device; GTX 1650 GPU has 4GB of VRAM, which is sufficient to train small to medium size BERT models, NLLB(for translation) and BAAI-BG3-1.5B-en for vectorisation provided Batching is used.
    \item Solution: For LLMs like Llama 3 and above, we might require over 12GB of VRAM which is consequently tied to the GPU.
...
\begin{itemize}
    \item Most smaller LLMs like Llama \& Deepseek 8B  and their contemporaries have a strict 512 token limit.
...
\begin{itemize}
    ...
    \item To create a Pipeline that takes court documents from both the opposing sides as inputs; cleans, translates and pre-processes them, passes them to later stages for creating outputs in line with the functional and non-functional requirements.
    \item To integrate Llama/Gemini/Deepseek for Legal code mapping of any codes/articles/statements found in the input documents
    \item To integrate a Vector DB for retrieving legal precedents from past cases. 
    \item To integrate Gemini for evidence scrutinization, verdict prediction and summarization of given court documents.
...
Model/Framework & Llama/Deepseek via DAPT and/or Gemini + RAG & Llama 3 and RAG & BERT and DAPT & Adversarial BERT and UDA \\ \hline
...
Interpretability & Medium & Low & Low & Low \\ \hline
...
Regulatory Focus & Family court aligned focus & Research protocols &  Commercial licensing & Commercial licensing \\ \hline
...
\begin{itemize}
    ...
    \item \textbf{GPU:} GTX 1650 or higher. RTX series/T4 GPUs with 12 GB VRAM or higher are preferred.
...
\begin{itemize}
	...
	\item \textbf{Easy/Hybrid OCR:} For parallel pre-processing to build dataset from AWS Registry PDFs.
    \item \textbf{NLLB: } For translation from other languages into English for downstream processes and vectorisation.
    \item \textbf{BAAI/bge-small-en-v1.5: } For vectorisation of built dataset and any input documents for Vector DB.
    \item \textbf{Llama 8B:} For more complex stages such as Evidence scrutinization, Summarization, and Verdict prediction.
    \item \textbf{Deepseek R1 Distill 8B:} For more complex stages such as Evidence scrutinization, Summarization, and Verdict prediction.	
    \item \textbf{Gemini API:} For more complex stages such as Evidence scrutinization, Summarization, and Verdict prediction.	
...
\label{sec:data_pipeline_methodology}

Figure~\ref{fig:data_pipeline} illustrates the data flow and component interactions within the JurisChain system.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\textwidth]{figs/DB_Detailed_Flow.pdf}
	\caption{End-to-End RAG Legal Data Pipeline Architecture.}
	\label{fig:data_pipeline}
\end{figure}

The system utilizes a specialized end-to-end RAG pipeline to transform raw judicial PDFs into an intelligent retrieval environment. The process is structured into four distinct stages:  
\begin{enumerate}
	\item \textbf{Ingestion \& Cleaning:} Raw legal PDFs from the AWS SC+HC Registry (1990-2025) are processed via multi-threaded execution using the script \texttt{batch\_converter.ipynb}. This stage leverages \textbf{PyMuPDF/Fitz} to extract raw text, which is then refined using regular expressions (regex) for structural noise removal.
	\item \textbf{Data Structuring:} Extracted text files and their corresponding JSON metadata files—which include details such as coram, date, and case numbers are merged using \textbf{parquet-creator-1990-2025.ipynb}. This compiles a consolidated Parquet dataset table to facilitate efficient data representation and entity fingerprinting.
	\item \textbf{Vectorization:} The structured Parquet dataset  undergoes recursive chunking and AI model vector embedding using parallel multi-GPU execution via \texttt{vector-embeddings.ipynb}. It employs the \textbf{BAAI/bge-small-en-v1.5} modelto generate a raw embeddings package consisting of a vector matrix (\texttt{sc\_vectors.npy}) and a textual payload (\texttt{sc\_payload.json}).
	\item \textbf{Retrieval \& Inference:} At runtime, the Streamlit application framework \texttt{my\_app.py} acts as the system builder and server. It encodes incoming user queries using the identical BAAI embedding model  to execute cosine similarity calculations against the persistent index stored in the \textbf{ChromaDB} vector database collection. This pipeline isolates and retrieves the top 10 most contextually relevant precedents to stage them for final LLM synthesis.
\end{enumerate}
...
\newpage
\section{Data Governance and Ethical Sourcing}
\label{sec:data_governance_ethical_sourcing}

Figure~\ref{fig:pipeline_architecture} outlines the corresponding structural data flow and runtime search query paths interacting with the storage sub-systems.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\textwidth]{figs/DB_Pipeline_Architecture.pdf}
	\caption{Data Flow and Storage Subsystem Architecture.}
	\label{fig:pipeline_architecture}
\end{figure}

To ensure complete architectural integrity, compliance, and user safety within the Legal Precedent Assistant (LPA) framework, the following data governance and ethical sourcing protocols are implemented across the pipeline:
\begin{itemize}
	\item \textbf{Public Judicial Corpus Sourcing:} The historical retrieval corpus is constructed exclusively using public records obtained via the AWS Registry for Supreme Court and High Court judgments covering the years 1990–2025. This baseline retrieval dataset complies with standard public information access parameters and academic research data exemptions.
	\item \textbf{Expert Legal Validation:} Structural pipeline rules, logic frameworks, and evidence cross-examination parameters were evaluated and calibrated using real-world qualitative inputs and target test samples collected directly from practicing legal professionals.
	\item \textbf{Strict Isolation of Private Test Samples:} While actual case records and validation samples were gathered from legal counsel to evaluate edge-case pipeline reliability, these documents are strictly restricted to transient runtime testing. They are completely excluded from the permanent ChromaDB vector store index and are never utilized during model domain-adaptive pre-training (DAPT) or fine-tuning stages. This procedural boundary eliminates the risk of leaking sensitive personal data or violating non-disclosure expectations in sensitive family matters.
	\item \textbf{Algorithmic Transparency:} To prevent ungrounded model hallucinations, the pipeline rejects end-to-end opaque logic. The core reasoning engines (Llama, Deepseek, Gemini) are restricted by strict prompt anchoring to force explicit source citations. Every predicted verdict, structural summary, or legal code mapping must display its explicit underlying source parameters retrieved from the verified database index.
\end{itemize}
...
Figure~\ref{fig:system_architecture} illustrates the data flow and component interactions within the JurisChain system.
...
\begin{itemize}
    ...
    \item \textbf{Gap 2 (Long Document Handling):} The implementation of Hierarchical Transformers (or sliding-window techniques) directly addresses the 512-token limit of standard BERT models and small LLMs, allowing for the processing of verbose Indian High Court and Supreme Court judgments.
    ...
    \item \textbf{Gap 6 (Semantic Accuracy): }  By utilizing LLMs and Vector-DB, the system captures complex legal jargon and intra-sentence relationships that traditional "Bag of Words" or basic frequency models fail to retrieve.
\end{itemize}
...
\begin{enumerate}
	...
	\begin{itemize}
		\item User uploads case documents such as petitions and affidavit of evidences from both parties.
		\item System extracts document types, court metadata, and case category (e.g., criminal, civil, etc).
	...
	\begin{itemize}
		\item PDFs are parsed using Easy OCR/PyMuPDF/Fitz to extract raw text from judgments and evidence.
		\item Text is cleaned (noise removal, normalization) and segmented into logical units (facts, arguments, evidence).
		\item An LLM such as Llama 8B, Deepseek R1(improved via DAPT) or Gemini is used to do initial synthesis of the documents.
	\end{itemize}
	
	
	\item \textbf{Precedent Retrieval via Vector DB:}
	\begin{itemize}
		\item The extracted pdf data is then vectorized using the BAAI/BG-3 model and the query is passed to Chroma DB.
		\item Chroma DB uses the vectorized query to find matching vectors/similar cases within it's database.
		\item The Precedents(past judgements) are then retrieved and converted from vector representations into a textual format(strings)
		\item These retrieved precedents are then fed to the LLM in next stages of the Pipeline/system.
	\end{itemize}
	
	
		\item \textbf{Initial Synthesis:}
	\begin{itemize}
		\item The LLM extracts basic data from the input pdfs.
		\item The initial synthesis includes name of both parties, important dates and primary allegations from both sides.
		\item Mapped IPC/BNS sections/acts/articles are also identified, but can change downstream based on more accurate data analysis and precedent alignment.
	\end{itemize}
	
	
	
	\item \textbf{Evidence Scrutinization:}
	\begin{itemize}
		\item Evidence affidavits from both sides are analyzed for contradictions and inconsistencies.
		\item Gemini/Deepseek R1 performs a consistency audit to evaluate logical coherence across claims.
		\item The LLM is also instructed to match the timeline and use alibis provided to check timeline consistency.
	\end{itemize}
	...
	\begin{itemize}
		\item Gemini consumes case facts, evidence scrutiny, mapped sections/acts/articles, and precedents.
	...
	\begin{itemize}
		\item Predicted verdict and reasoning are simplified
...
\begin{itemize}
	...
	Contradiction detection and consistency auditing using Gemini are computationally intensive but feasible under a controlled pipeline. The system operates on complete evidence docs to maintain context, significantly increasing token length and inference cost to maintain accuracy.
	
	\item \textbf{Precedent Retrieval and Similar Case Matching:}
	Precedent retrieval is implemented using a Vector Database such as Chroma DB. BAAI/BG-3 embeddings stored in vector databases enable fast approximate nearest neighbor search even for large precedent collections. This approach scales linearly with data size and has been validated in prior legal IR research.
	
	\item \textbf{Verdict Prediction and Summarization:}
	Verdict and sentence prediction using Gemini leverages structured inputs (facts, mapped sections, precedents), reducing hallucination risk. Summarization under a fixed word limit is a well-established task for large language models and can be executed with deterministic decoding strategies to ensure consistency.
	
	\item \textbf{Output Accuracy:}
	Since the primary domain is Indian Family courts, the output from all the stages/Langchain prompts may not be fully accurate on account of an LLM not fully understanding the nuances of human relationships and emotions, in this regard a actual Human expert such as Judge, Lawyer, Attorney, etc, will always be preferred.
\end{itemize}
...
\begin{table}[h]
	...
	\begin{tabular}{|l|l|p{6cm}|}
		...
		\textbf{Phase 1: Data Collection \& Preprocessing} & Weeks 1--4 & Curated dataset of trial and high court judgments; PDF-to-text pipeline using Fitz; cleaned and segmented legal text; collection of real word test samples from Lawyers. \\ \hline
		\textbf{Phase 2: DAPT and Model Fine-tuning} & Weeks 5--8 & 
		Perform DAPT on Llama and Deepseek R1 models and perform tests on them using sample documents. \\ \hline
		\textbf{Phase 3: Evidence Scrutinization Module} & Weeks 9--12 & Contradiction detection and truth-scoring logic implemented using Gemini; structured evidence representation. \\ \hline
		\textbf{Phase 4: Precedent Retrieval System} & Weeks 13--16 & Vector-Embedding-based retrieval; vector database of precedents; ranked precedent retrieval. \\ \hline
		\textbf{Phase 5: Verdict Prediction \& Summarization} & Weeks 17--20 & Verdict and sentence prediction pipeline using Gemini; simplified summary generation under 150 words. \\ \hline
...
The critical dependency chain progresses through reliable data preprocessing (Phase 1) → DAPT and Model Fine-tuning (Phase 2) → evidence scrutinization (Phase 3). Errors in early-stage legal text processing propagate downstream and directly affect verdict prediction quality. Buffer time is allocated in Phase 6 to address cumulative model errors and integration issues.

\textbf{Resource Availability:}
The project is executed by a single postgraduate student with guidance from a faculty advisor. Development relies on open-source libraries (Legal-BERT, Hugging-Face, Llama, Deepseek, Kaggle, Google Collab, Chroma DB, NLLB, BAAI-BG, Streamlit UI, PyMuPDF/Fitz) and institutional computing resources. Model inference is scheduled offline or in batch mode, ensuring feasibility without continuous high-end GPU availability.

\textbf{Schedule Risks:}
\begin{itemize}
	\item \textbf{Sample Data Procurement Issue:} Delay in procuring sample court data from Lawyers may delay Phase 6. 
	Mitigation: Restrict scope to digitally available judgments.
	\item \textbf{Data Quality Issues:} Inconsistent formatting or OCR errors in court documents may delay Phase 1. Mitigation: Restrict scope to digitally available judgments.
	\item \textbf{Model Integration Complexity:} Coordinating multiple NLP models (Deepseek, LLaMA, NLLB) may increase integration time. Mitigation: Modular pipeline design with independently testable components.
...
\begin{itemize}
	...
	The system is developed using open-source libraries and pretrained models such as Llama, Deepseek, NLLB, PyMuPDF (Fitz), and Chroma DB, all of which permit academic and research use under permissive licenses (e.g., Apache 2.0, MIT). The project is intended strictly for non-commercial academic purposes, ensuring compliance with licensing terms.
	
	\item \textbf{Model Usage Rights:}
	Pretrained language models (e.g., Deepseek, LLaMA, Gemini API) are used for inference and fine-tuning in accordance with their respective research-use licenses. No proprietary datasets or restricted commercial legal databases are incorporated, avoiding licensing conflicts.
	
	\item \textbf{Original Contributions:}
	The architectural design, sample test data collection, evidence scrutinization, and pipeline integration constitute original academic work. While certain components (e.g., contradiction-aware verdict prediction) may have future commercial potential, no patent claims are pursued during the academic phase.
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Sample Test Data Usage:}
	Family court samples are used only to test and evaluate the system and are not to be used as part of the Training Database for LLMs nor as part of the Vector Database due to presence of sensitive and personally identifiable data.
...
NLP models trained on historical legal data may inherit systemic biases present in past judgments. To mitigate this risk, the system incorporates explainable intermediate outputs (evidence cross-examination, law code mappings, retrieved precedents) rather than opaque end-to-end predictions. This supports transparency and allows human users to critically evaluate model outputs.
...
\begin{table}[h]
	...
	\begin{tabular}{|l|c|p{6.5cm}|}
		...
		Technical & Moderate-High & Mature NLP models (Legal-BERT, LLaMA 3); modular pipeline; no real-time constraints apart from Dataset acquisiton. \\ \hline
